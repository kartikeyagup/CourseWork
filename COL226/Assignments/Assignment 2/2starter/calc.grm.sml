...
datatype svalue = VOID | ntVOID of unit ->  unit
 ...
 | START of unit ->  (Absyn.node option)
 | EXP of unit ->  (Absyn.node)
end
type svalue = MlyValue.svalue
type result = Absyn.node option
end
structure EC=
struct
open LrTable
...
|  ( 4, ( ( _, ( MlyValue.ID ID1, ID1left, ID1right)) :: rest671)) =>
 ...
 in (Absyn.NODE ("lookup", [Absyn.LEAFS ID]))
end)
...
MlyValue.EXP (fn _ => let val  EXP1 = EXP1 ()
 val  EXP2 = EXP2 ()
 in (Absyn.NODE ("divide", [EXP1, EXP2]))
end)
 in ( LrTable.NT 0, ( result, EXP1left, EXP2right), rest671)
end
...
MlyValue.EXP (fn _ => let val  EXP1 = EXP1 ()
 val  EXP2 = EXP2 ()
 in (Absyn.NODE ("subtract", [EXP1, EXP2]))
end)
 in ( LrTable.NT 0, ( result, EXP1left, EXP2right), rest671)
end
