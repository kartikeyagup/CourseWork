...
string TreeNode::LowestCommonBoss(string emp1,string emp2)
{
	...
	if (emp1.compare(emp2)==0) cout << "The 2 names are the same\n";
	else if (empnode1==NULL || empnode2==NULL) return "Incorrect Input";
	else
	{
		//Reduce steps
		vector<string> e1,e2;
        ...
        int lim1=e1.size(), counter1=lim1-1, lim2=e2.size(), counter2=lim2-1;
        bool found1=false;
        ...
        if (lim1*lim2==0) return "No Common Boss";
        else return e1[counter1+1]; 
	}
}
...
TreeNode* TreeNode::DeleteEmployee(string deleteemp, string tomoveto)
{
	...
	else if (positiondelete == positionappend) cout <<" The 2 names are the same \n";
	else
	{
